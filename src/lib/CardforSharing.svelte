    ...
    async function commitSharing(){
        ...
        for(let i=0; i<filesToPick.length; i++){
            if(filesToPick[i].checked){
                try{
                    console.log("Params:", $contract, $account, receiverAddr, $myFiles[i].cid, 1);
                    var [stats, res] = await commitFilePermission($contract, $account, receiverAddr, $myFiles[i].cid, 1)
                }
                finally{
                    if(stats){
                        output.push(res);
...
<div class="card p-4" style="width:fit-content;">
    ...
    <form on:submit={commitSharing}>
        <input class="input" style="margin-top: 10px; margin-bottom: 10px;" type="text" bind:value={receiverAddr} placeholder="Enter Receiver's address."/>
    </form>
    {#if loading}
	    <ProgressBar/>  
    {/if}
